...
public class BooleanOrNullLiteralInAssertionsCheck extends AbstractMethodDetection {
  ...
  private void checkEqualityAsserts(MethodInvocationTree mit, boolean flipped) {
    ...
    if (arity == 3) {
      expected = mit.arguments().get(1);
      actual = mit.arguments().get(2);
    } else if (arity == 2) {
      expected = mit.arguments().get(0);
      actual = mit.arguments().get(1);
    } else {
      return;
    }
    if (isBoolOrNullLiteral(expected) && isBoolOrNullLiteral(actual)) {
      reportIssue(mit, DEFAULT_MESSAGE);
    } else if (isBoolOrNullLiteral(expected)) {
      checkEqualityAssertWithOneLiteral(mit, expected, flipped, ASSERT);
    } else if (isBoolOrNullLiteral(actual)) {
      checkEqualityAssertWithOneLiteral(mit, actual, flipped, ASSERT);
    }
  }
  ...
  private void checkFestEqualityAsserts(MethodInvocationTree mit, boolean flipped) {
    ...
    if (mit.arguments().isEmpty() || actual == null) {
      return;
    }
